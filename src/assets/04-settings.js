  ...
  window.renderSettings=function(){
    ...
    el.innerHTML=`<div class="two settings-top-grid">
      <div class="settings-left-stack">
        ...
        <div class="card"><h2>⚡ Performance</h2><p class="muted">Native SQLite is the long-life record store for the Windows app: WAL mode, indexed date queries, background writes and bounded list/search APIs. IndexedDB remains for attachment blobs and browser fallback; large synchronous localStorage writes stay off the critical path.</p></div>
        <div class="card"><h2>🗄️ Long-life Database</h2><p class="muted">ॐ keeps records in a native SQLite database instead of depending on browser localStorage size. Data is versioned, transaction-safe and indexed for large histories.</p><div class="row"><button class="secondary" type="button" onclick="window.checkOmDatabase()">Check database</button><span id="omDbStatus" class="meta">Checking…</span></div></div>
      </div>
