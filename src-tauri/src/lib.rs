...
fn set_db_folder(app: &AppHandle, folder: &Path) -> Result<(), String> {
    if !folder.is_dir() {
        return Err("Selected database folder does not exist.".into());
    }

    let config = db_config_path(app)?;
    let raw = serde_json::to_string(&folder.to_string_lossy().into_owned())
...
fn db_upsert_record(
    ...
    conn.execute(
        "INSERT INTO records(section,record_id,date_key,updated_at,payload)
         VALUES(?1,?2,?3,?4,?5)
...
fn db_delete_record(
    ...
    let n = conn
        .execute(
            "DELETE FROM records WHERE section=?1 AND record_id=?2",
            params![section, record_id],
        )
        .map_err(|e| e.to_string())?;
...
fn db_delete_records(
    ...
    let mut stmt = tx
        .prepare("DELETE FROM records WHERE section=?1 AND record_id=?2")
        .map_err(|e| e.to_string())?;
...
fn online_backup(app: &AppHandle, target: &PathBuf) -> Result<String, String> {
    ...
    // IMPORTANT: use SQLite's Online Backup API rather than copying
...
fn db_backup_to(
    ...
    if target.as_os_str().is_empty() {
        return Err("Backup target is empty".into());
    }
...
fn db_auto_backup(
    ...
    let database_root = db_path(&app)?
        .parent()
        .map(PathBuf::from)
        .ok_or_else(|| "Database folder unavailable.".to_string())?;
    ...
    let previous_stamp: i64 = {
        ...
        conn.query_row(
            "SELECT CAST(value AS INTEGER)
             FROM meta
             WHERE key='last_auto_backup_stamp'",
            [],
            |r| r.get(0),
        )
        .unwrap_or(0)
...

    {
        ...
        conn.execute(
            "INSERT INTO meta(key,value,updated_at)
             VALUES('last_auto_backup_stamp',?1,?2)
             ON CONFLICT(key)
             DO UPDATE SET
                 value=excluded.value,
                 updated_at=excluded.updated_at",
            params![
                current_stamp.to_string(),
                stamp.as_secs() as i64
            ],
        )
        .map_err(|e| e.to_string())?;
    }
...
fn db_list_records(
    ...
    let has_cursor =
        cursor_updated_at.is_some() && cursor_record_id.is_some();
    ...
    let mut collect =
        |sql: &str, bind: &[&dyn rusqlite::ToSql]| -> Result<(), String> {
            let mut st =
    ...
    match (from_date, to_date, has_cursor) {
        (Some(from), Some(to), true) => collect(
            "SELECT section,record_id,date_key,updated_at,payload
             FROM records
             WHERE section=?1
               AND date_key>=?2
               AND date_key<=?3
             ...
             ORDER BY date_key DESC, updated_at DESC, record_id DESC
             LIMIT ?7",
            &[
                &section,
                &from,
...
fn db_list_record_ids(
    ...
    let mut st = conn
        .prepare("SELECT record_id FROM records WHERE section=?1")
        .map_err(|e| e.to_string())?;

    let rows = st
        .query_map(params![section], |r| r.get::<_, String>(0))
        .map_err(|e| e.to_string())?;
...
fn db_count_records(
    ...
    conn.query_row(
        "SELECT COUNT(*) FROM records WHERE section=?1",
        params![section],
        |r| r.get(0),
    )
    .map_err(|e| e.to_string())
...
fn db_search_records(
    ...
    let rows = st
        .query_map(params![section, q, limit], |r| {
            Ok(DbRecord {
                section: r.get(0)?,
                record_id: r.get(1)?,
                date_key: r.get(2)?,
...
fn db_get_record(
    ...
    conn.query_row(
        "SELECT section,record_id,date_key,updated_at,payload
         FROM records
         WHERE section=?1 AND record_id=?2",
        params![section, record_id],
        |r| {
            Ok(DbRecord {
                section: r.get(0)?,
...
fn db_finance_summary(
    ...
    let sum_section = |section: &str| -> Result<f64, String> {
        ...
        let rows = st
            .query_map(params![section, from_date, to_date], |r| {
                r.get::<_, String>(0)
            })
            .map_err(|e| e.to_string())?;
...
fn validate_backup_file(path: &Path) -> Result<(), String> {
    ...
    let conn =
        Connection::open(path).map_err(|e| format!("Could not open backup: {e}"))?;
    ...
    if integrity != "ok" {
        return Err(format!(
            "Selected backup failed integrity check: {integrity}"
        ));
    }
...
fn preserve_live_database(path: &Path) -> Result<PathBuf, String> {
    ...
    for suffix in ["", "-wal", "-shm"] {
        ...
        if src.is_file() {
            ...
            fs::copy(&src, quarantine.join(name))
                .map_err(|e| {
                    format!(
                        "Could not preserve damaged database file {name}: {e}"
                    )
                })?;
    ...
    if !copied {
        return Err("Live database file was not found.".into());
    }
...
fn choose_database_backup(
    ...
    #[cfg(windows)]
    {
        let default_dir = app
            ...
            .document_dir()
            .ok()
            .map(|p| p.join("Om-LifeOS").join("DatabaseBackups"));

        let mut dialog =
            rfd::FileDialog::new()
                .set_title("Choose verified LifeOS SQLite backup");
    ...
    #[cfg(not(windows))]
    {
        ...
        Err(
            "Native database backup picker is currently available on Windows."
                .into(),
        )
    }
}
...
fn db_restore_from_backup(
    ...
    let source = Connection::open(&backup_path)
        .map_err(|e| format!("Could not open verified backup: {e}"))?;
    ...
    {
        let backup_op =
            Backup::new(&source, &mut *live)
                .map_err(|e| {
                    format!(
                        "Could not restore backup into live database: {e}"
                    )
                })?;
    ...
    if integrity != "ok" {
        return Err(format!(
            "Restore completed but integrity check failed: {integrity}"
        ));
    }

    let records: i64 = live
        .query_row("SELECT COUNT(*) FROM records", [], |r| r.get(0))
        .map_err(|e| e.to_string())?;

    Ok(DbRestoreResult {
        restored_from: backup_path.to_string_lossy().into_owned(),
        preserved_damaged_copy: preserved.to_string_lossy().into_owned(),
        integrity,
        records,
    })
}
...
// FIX 1:
// This is a struct, so it must NOT have #[tauri::command].
#[derive(Debug, Serialize)]
...
#[tauri::command]
fn choose_database_folder(
    app: AppHandle,
) -> Result<Option<DatabaseFolderResult>, String> {
    #[cfg(windows)]
    {
        ...
        let mut dialog =
            rfd::FileDialog::new()
                .set_title("Choose LifeOS long-life database folder");
        ...
        if !same && target.exists() {
            return Err(
                "Selected folder already contains om-lifeos.sqlite3. Choose an empty LifeOS folder to avoid overwriting another database."
                    .into(),
            );
        }
        ...
        if !same && current.is_file() {
            ...
            let conn =
                Connection::open(&target).map_err(|e| e.to_string())?;
    ...
    #[cfg(not(windows))]
    {
        ...
        Err(
            "Native database folder picker is currently available on Windows."
                .into(),
        )
    }
}
...
// FIX 2:
// This function IS registered in generate_handler!,
// so it needs #[tauri::command].
#[tauri::command]
fn choose_export_directory() -> Result<Option<String>, String> {
    ...
    {
        Err(
            "Native Windows folder picker is only available on Windows."
                .into(),
        )
    }
}
...
fn save_export_file(
    ...
    if !root_path.is_dir() {
        return Err(
            "Connected export folder no longer exists.".into()
        );
    }
