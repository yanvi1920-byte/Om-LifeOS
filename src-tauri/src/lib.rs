...
#[tauri::command]
async fn db_backup_to(app: AppHandle, target: String) -> Result<String, String> {
    let target = PathBuf::from(target);
    if target.as_os_str().is_empty() { return Err("Backup target is empty".into()); }
    // SQLite backup is blocking I/O. Keep it off the async/UI path.
    tauri::async_runtime::spawn_blocking(move || online_backup(&app, &target))
        .await
        .map_err(|e| format!("Backup task failed: {e}"))?
}

#[tauri::command]
async fn db_auto_backup(app: AppHandle, state: State<'_, DbState>) -> Result<String, String> {
    ...
    // Large SQLite snapshots are blocking I/O; keep them off the async/UI path.
    let saved = tauri::async_runtime::spawn_blocking({
        let app = app.clone();
        let target = target.clone();
        move || online_backup(&app, &target)
    })
    .await
    .map_err(|e| format!("Automatic backup task failed: {e}"))??;
...
#[tauri::command]
async fn android_request_storage_permission() -> Result<bool, String> {
    Err("Android storage permission is available only in the Android app.".into())
}

#[cfg(not(target_os = "android"))]
#[tauri::command]
async fn android_pick_storage_folder() -> Result<Option<String>, String> {
    Err("Android storage folder picker is available only in the Android app.".into())
}

#[cfg(not(target_os = "android"))]
#[tauri::command]
async fn android_save_export_file(_folder_uri: String, _subfolder: String, _filename: String, _bytes: Vec<u8>) -> Result<String, String> {
    Err("Android storage export is available only in the Android app.".into())
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn android_request_storage_permission(app: AppHandle) -> Result<bool, String> {
...
#[cfg(target_os = "android")]
#[tauri::command]
async fn android_pick_storage_folder(app: AppHandle) -> Result<Option<String>, String> {
    use tauri_plugin_android_fs::AndroidFsExt;
    let api = app.android_fs_async();
    let selected = api.file_picker().pick_dir(None).await.map_err(|e| e.to_string())?;
    let Some(uri) = selected else { return Ok(None); };
    api.take_persistable_uri_permission(&uri).await.map_err(|e| e.to_string())?;
    Ok(Some(uri.to_json_string().map_err(|e| e.to_string())?))
}

#[cfg(target_os = "android")]
#[tauri::command]
async fn android_save_export_file(
    app: AppHandle,
    folder_uri: String,
    subfolder: String,
    filename: String,
    ...
    if safe_filename != filename || safe_filename.is_empty() { return Err("Invalid export filename.".into()); }
    let folder_name = subfolder.trim();
    if !matches!(folder_name, "Word" | "Excel" | "PDF" | "Backup" | "") { return Err("Invalid export folder.".into()); }
    let folder = FsUri::from_json_str(&folder_uri).map_err(|e| format!("Invalid connected storage folder: {e}"))?;
    let api = app.android_fs_async();
    let relative = if folder_name.is_empty() { safe_filename.to_string() } else { format!("Om-LifeOS/{folder_name}/{safe_filename}") };
    let mime = match Path::new(safe_filename).extension().and_then(|x| x.to_str()).unwrap_or("").to_ascii_lowercase().as_str() {
    ...
    let file_uri = api.create_new_file(&folder, &relative, mime).await.map_err(|e| e.to_string())?;
    api.write(&file_uri, &bytes).await.map_err(|e| e.to_string())?;
    Ok(file_uri.to_json_string().map_err(|e| e.to_string())?)
}
...
fn save_export_file(
    ...
    // Atomic export: write beside the final file, flush/close it, then rename.
    // A crash during the write therefore cannot leave a partially-written final file.
    let tmp = target.with_file_name(format!(".{}.{}.tmp", safe_filename, std::process::id()));
    let result = (|| -> Result<(), String> {
        use std::io::Write;
        let mut file = fs::File::create(&tmp).map_err(|e| format!("Could not create temporary export: {}", e))?;
        file.write_all(&bytes).map_err(|e| format!("Could not write {}: {}", safe_filename, e))?;
        file.sync_all().map_err(|e| format!("Could not flush {}: {}", safe_filename, e))?;
        drop(file);
        if target.exists() { fs::remove_file(&target).map_err(|e| format!("Could not replace {}: {}", safe_filename, e))?; }
        fs::rename(&tmp, &target).map_err(|e| format!("Could not finalize {}: {}", safe_filename, e))?;
        Ok(())
    })();
...
pub fn run() {
    ...
    builder
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            android_request_storage_permission, android_pick_storage_folder, android_save_export_file
        ])
